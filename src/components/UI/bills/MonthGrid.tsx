  ...
  return (
    ...
    <div className="grid grid-cols-7 gap-2 text-sm">
      ...
      {cells.map((d, i) => (
        <div
          key={i}
          className="h-28 rounded-xl border border-slate-200 p-2 flex flex-col bg-white"
        >
            ...
            <>
              ...
              <div className="space-y-1 overflow-auto">
                {(itemsByDay.get(d.getDate()) || []).slice(0, 3).map((it) => (
                  <div 
                    key={it.id} 
                    className={`rounded-lg px-2 py-1 text-xs flex items-center justify-between ${it.overdue ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-700"}`}
                  >
