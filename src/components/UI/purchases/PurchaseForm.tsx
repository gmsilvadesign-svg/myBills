      ...
      <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl p-6 w-full max-w-3xl shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Input label={t.title || 'Título'} value={title} onChange={e => setTitle(e.target.value)} autoFocus ref={titleRef as any} />
            <Input label={t.amount || 'Valor'} type="number" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} required />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
