...
export default function SettingsModal({ open, onClose, prefs, setPrefs, t }) {
  ...
  // Reseta valores ao abrir/fechar modal
  useEffect(() => {
    if (!open) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-lg shadow-xl">
        ...
        <div className="flex gap-3 justify-end mt-6">
          <button onClick={onClose} className="px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-800">{t.close}</button>
          <button onClick={() => { setPrefs((p)=>({ ...p, theme, language })); onClose(); }} className="px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900">{t.save}</button>
        </div>
      </div>
    </div>
  );
